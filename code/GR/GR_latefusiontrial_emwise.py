#late fusion results for emwise GR
...
#     f1train[experiment] = np.zeros([foldNum,])
...
if __name__ == '__main__':
    for experiment in experiments:
        ...
        f1Both[experiment] = np.zeros([foldNum,])
        f1train = 0
        for em in [2,4]:#range(0,len(emotion)):
            ...
            # model_save = ModelCheckpoint(experiment+'weights.hdf5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
                ...
                for alpha in alpha_range:
                    ...
                    # f1train = recall_score(labels[train,], po_train[:,1]>po_train[:,0],average='macro')
            ...
            sio.savemat('./temp/'+experiment+emotion[em]+'_forfusion_10.mat', {'accNet': accNet[experiment], 'accAda': accAda[experiment], 'accBoth': accBoth[experiment], 'aucNet': accNet[experiment], 'aucAda': aucAda[experiment], 'aucBoth': aucBoth[experiment]
