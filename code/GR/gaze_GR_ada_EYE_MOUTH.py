...
with open('GR_ada_gaze_results2_log.csv', 'a') as csvfile:
...
for n_est in [10]:
	for em in range(1,7):
		for experiment in experiments:
			...
			allgenderMat = np.squeeze(np.ones([allfeatures.shape[0],1]))
			matContent = sio.loadmat(dataPath+'female_feat')
			allfeatures = np.append(allfeatures,matContent['featureMat'],axis=0)
			alllabels = np.append(alllabels,matContent['labelMat'],axis=0)
			allgenderMat = np.append(allgenderMat,np.squeeze(np.zeros([matContent['featureMat'].shape[0],1])),axis=0)
			# print allgenderMat.shape
			if experiment==experiments[0]:
				idx = np.squeeze(np.intersect1d(np.where(alllabels[:,4]==1),np.squeeze(np.intersect1d(np.where(alllabels[:,3]==em),np.squeeze(np.intersect1d(np.where(alllabels[:,0]==1),
					np.where(alllabels[:,1]==alllabels[:,3])))))))
				...
				idx = np.squeeze(np.intersect1d(np.where(alllabels[:,4]==0),np.squeeze(np.intersect1d(np.where(alllabels[:,3]==em),np.squeeze(np.intersect1d(np.where(alllabels[:,0]==1),
					np.where(alllabels[:,1]==alllabels[:,3])))))))
				features = allfeatures[idx,]
				labels = allgenderMat[idx]
