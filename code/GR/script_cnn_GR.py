# GR and VR using CNN with EEG data
import os
...
dataPath = './data/'
savePath = './Results/'
experiments = ['Gender_Clean', 'Gender_notClean'] #  
...
pool_size = 3
stride_size = 2
nb_epoch = 30#[10, 10, 10]  
learn_rate = 0.01
batch_size = 10
mtm = 0.9
padding = 'same'
dense_layer_neuron_num = 8
dropout_level = 0.1
...
for experiment in experiments:
    ...
    if (classifier == 'Both'):
        if (experiment == 'Gender_Clean' or experiment == 'Gender_notClean'):
            sio.savemat(savePath + experiment + '_Results_CNN' + '.mat', {'precisionNet': precisionNet,'recallNet': recallNet, 'f1Net': f1Net, 'accNet': accNet,'aucNet': aucNet,
                                                    'precisionSVM': precisionSVM,'recallSVM': recallSVM, 'f1SVM': f1SVM, 'accSVM': accSVM, 'aucSVM': aucSVM})
        elif (experiment == 'Emotion_Clean' or experiment == 'Emotion_notClean'):
            sio.savemat(savePath + experiment + classifier + '_Results_CNN' + '.mat', {'accNet': accNet, 'accSVM': accSVM, 'accNet': accNet, 'aucNet': aucNet, 'aucSVM': aucSVM})
    elif (classifier == 'CNN'):
        if (experiment == 'Gender_Clean' or experiment == 'Gender_notClean'):
            sio.savemat(savePath + experiment + '_Results_CNN' + '.mat', {'precisionNet': precisionNet,'recallNet': recallNet, 'f1Net': f1Net, 'accNet': accNet, 'aucNet': aucNet})
        elif (experiment == 'Emotion_Clean' or experiment == 'Emotion_notClean'):
            sio.savemat(savePath + experiment + classifier + '_Results_CNN' + '.mat', {'accNet': accNet, 'aucNet': aucNet})
    elif (classifier == 'SVM'):
        if (experiment == 'Gender_Clean' or experiment == 'Gender_notClean'):
            sio.savemat(savePath + experiment + '_Results__CNN' + '.mat', {'precisionSVM': precisionSVM,'recallSVM': recallSVM, 'f1SVM': f1SVM, 'accSVM': accSVM, 'aucSVM': aucSVM})
        elif (experiment == 'Emotion_Clean' or experiment == 'Emotion_notClean'):
            sio.savemat(savePath + experiment + classifier + '_Results_CNN' + '.mat', {'accSVM': accSVM, 'aucNet': aucNet})
