# GR and VR with CNN EEG features for HI/LI/EYE/MOUTH mask conditions
import os
        ...
        for em in range(n_em):
            ...
            for train, test in skf.split(fold_features, fold_labels):
                ...
                if (classifier == 'CNN' or classifier == 'Both'):
                    ...
                    model_save = ModelCheckpoint(experiment+'weights.{epoch:02d}-{val_loss:.2f}.hdf5', monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
                    for ep in range(len(nb_epoch)):
                        ...
                        model.fit(trainingFeatures, labels_categorical[train,:], batch_size=batch_size, epochs=nb_epoch[ep],
                                  verbose=0, callbacks=[earlyStopping,model_save], validation_split=0.1) 
                    # sgd = adam()
                    # model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
                    # model.summary()
                ...
                if (classifier == 'SVM' or classifier == 'Both'):
                    ...
                    if (experiment == 'Gender_Clean' or experiment == 'Gender_notClean'):
                        ...
                        clf.fit(np.reshape(trainingFeatures, [train_shape[0], train_shape[1]*train_shape[2]]), fold_labels[train,]) 
                        ...
                        precisionSVM[f] = precision_score(fold_labels[test,], predicted_labelsSVM)
                        recallSVM[f] = recall_score(fold_labels[test,], predicted_labelsSVM)
                        f1SVM[f] = f1_score(fold_labels[test,], predicted_labelsSVM, average='macro')
                        accSVM[f] = accuracy_score(fold_labels[test,], predicted_labelsSVM)
                        aucSVM[f] = roc_auc_score(fold_labels[test,], predicted_probsSVM[:,1])
            ...
            if (classifier == 'Both'):
                if (experiment == 'Gender_Clean_HI' or experiment == 'Gender_notClean_HI'):
                    sio.savemat(savePath + experiment + '_Results_HI_'+ em_list[em] + '.mat', {'precisionNet': precisionNet,'recallNet': recallNet, 'f1Net': f1Net, 'accNet': accNet,'aucNet': aucNet,
                    ...
                    sio.savemat(savePath + experiment + '_Results_LI_'+ em_list[em] + '.mat', {'precisionNet': precisionNet,'recallNet': recallNet, 'f1Net': f1Net, 'accNet': accNet,'aucNet': aucNet,
                                                            'precisionSVM': precisionSVM,'recallSVM': recallSVM, 'f1SVM': f1SVM, 'accSVM': accSVM, 'aucSVM': aucSVM})
                elif (experiment == 'Gender_Clean_EYE' or experiment == 'Gender_notClean_EYE'):
                    sio.savemat(savePath + experiment + '_Results_EYE_'+ em_list[em] + '.mat', {'precisionNet': precisionNet,'recallNet': recallNet, 'f1Net': f1Net, 'accNet': accNet,'aucNet': aucNet,
                                                            'precisionSVM': precisionSVM,'recallSVM': recallSVM, 'f1SVM': f1SVM, 'accSVM': accSVM, 'aucSVM': aucSVM})
                elif (experiment == 'Gender_Clean_MOUTH' or experiment == 'Gender_notClean_MOUTH'):
                    sio.savemat(savePath + experiment + '_Results_MOUTH_'+ em_list[em] + '.mat', {'precisionNet': precisionNet,'recallNet': recallNet, 'f1Net': f1Net, 'accNet': accNet,'aucNet': aucNet,
                                                            'precisionSVM': precisionSVM,'recallSVM': recallSVM, 'f1SVM': f1SVM, 'accSVM': accSVM, 'aucSVM': aucSVM})
                elif (experiment == 'Emotion_Clean_HI' or experiment == 'Emotion_notClean_HI'):
                    sio.savemat(savePath + experiment + classifier + '_Results_HI_'+ em_list[em] + '.mat', {'accNet': accNet, 'accSVM': accSVM, 'accNet': accNet, 'aucNet': aucNet, 'aucSVM': aucSVM})
            elif (classifier == 'CNN'):
                if (experiment[0][0] == 'G'):
                    sio.savemat(savePath + experiment + '_Results_'+ em_list[em] + '.mat', {'precisionNet': precisionNet,'recallNet': recallNet, 'f1Net': f1Net, 'accNet': accNet, 'aucNet': aucNet})
                # elif (experiment == 'Gender_Clean_LI' or experiment == 'Gender_notClean_LI'):
