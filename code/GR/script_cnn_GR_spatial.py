...
dataPath = './../DNN/Data/'
savePath = '/home/maneesh/Downloads/Code/spatial/'#Results/'
# experiments = ['Gender_notClean_HI', 'Gender_notClean_LI', 'Gender_notClean_EYE','Gender_notClean_MOUTH'] #  
experiments = [ 'Gender_notClean_MOUTH']#, 'Gender_notClean_LI', 'Gender_notClean_EYE', 'Gender_notClean_MOUTH'] #  
...
auc_spatstd = {experiments[0]:None}#,experiments[1]:None,experiments[2]:None,experiments[3]:None}
auc_spat = {experiments[0]:None}#,experiments[1]:None,experiments[2]:None,experiments[3]:None}

for experiment in experiments:
    auc_spat[experiment] = []
    auc_spatstd[experiment] = []
    # Loading data
    ...
    features_temp = features[rand_order,]
    labels = np.squeeze(labels[rand_order,])
    for electrode in range(0,14):
        print 'electrode-{0}'.format(electrode)
        features = np.reshape(features_temp[:,:,electrode],[features_temp.shape[0],features_temp.shape[1],1])
        if (experiment[0][0] == 'G'):
        ...
        if (classifier == 'CNN'):
            if (experiment[0][0] == 'G'):
                sio.savemat(savePath + experiment +'e'+str(electrode)+ '.mat', {'precisionNet': precisionNet,'recallNet': recallNet, 'f1Net': f1Net, 'accNet': accNet, 'aucNet': aucNet})
        auc_spat[experiment].append(np.mean(aucNet))
        auc_spatstd[experiment].append(np.std(aucNet))
        exp += 1
    sio.savemat(savePath + experiment + '.mat', {'auc_spat': auc_spat,'auc_spatstd':auc_spatstd})